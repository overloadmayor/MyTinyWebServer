...
#include <sys/types.h>
#include <unistd.h>
int startcnt = 0, endcnt = 0, sumcnt = 0;
WebServer::WebServer() {
  // http_conn类对象
...
void WebServer::eventListen() {
  ...
  utils.addfd(m_epollfd, m_listenfd, false, m_LISTENrigmode);
...
void WebServer::deal_timer(util_timer *timer, int sockfd) {
  // printf("deal_timer start\n");
  timer->cb_func(&users_timer[sockfd]);
  if (timer) {
    // printf("delete util_timer:%d\n", timer->user_data->sockfd);
...
void WebServer::dealwithread(int sockfd) {
  util_timer *timer = users_timer[sockfd].time;
  // reactor
  if (1 == m_actormodel) {
    if (timer) {
      adjust_timer(timer);
